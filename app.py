...
lineaccesstoken = '26Xm+yjmSIdjfWJ9gkwCNNSXQdo+1WOFYtPgqWs6Qt1z4Uv4jcPe5V6wpJUXoTvMcCn1fWCr6pbbxlT5j27JVfeJ1KJbL0HmfSOCxz7wxZuRZigVnNHu8j7AuI2KE8JFBX69Sg4HOKty68pI8AjDAwdB04t89/1O/w1cDnyilFU='
...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if msg == "สวัสดี" :
            replyObj = TextSendMessage(text="จร้า ไหว้พระเถอะลูก")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "กินข้าวกัน" :
            replyObj = TextSendMessage(text="กินก่อนเลย ยังไม่หิว")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "หมดโควิดจะไปไหนดี" :
            replyObj = TextSendMessage(text="ไปทั่วโลกเลย 555")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
            headers = request.headers
            json_headers = ({k:v for k, v in headers.items()})
            json_headers.update({'Host':'bots.dialogflow.com'})
            url = "https://bots.dialogflow.com/line/f951538a-9ce9-47d9-9f53-54249d759096/webhook"
